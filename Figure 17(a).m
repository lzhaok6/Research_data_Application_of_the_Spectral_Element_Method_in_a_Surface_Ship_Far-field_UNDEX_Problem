...
%     end
% end
%bench_N5_02ft2(:,1)=bench_N5_02ft2(:,1)-1.5;
% plot(bench_N5_02ft(:,1), bench_N5_02ft(:,2),'-r','LineWidth',0.5);
% hold on
% plot(DDG_2ft3(:,1), DDG_2ft3(:,2),'-r','LineWidth',0.5);
% hold on
% plot(DDG_2ft4(:,1), DDG_2ft4(:,2),'-b','LineWidth',0.5);
% hold on
% plot(hex_05ft(:,1), hex_05ft(:,2),'-b','LineWidth',0.5);
% hold on
% plot(Structured1ft_mirror(:,1), Structured1ft_mirror(:,2),'-r','LineWidth',0.5);
% hold on
a=0.3048;
ce=1500; 
rhoe=1025; 
P1=1; 
% front_v1_norm(:,1)=front_v1(:,1)/(a/ce);
% front_v1_norm(:,2)=-front_v1(:,2)*(rhoe*ce/P1);
% plot(front_v1_norm(:,1), front_v1_norm(:,2),'-r','LineWidth',0.5);
% hold on
% plot(Benchmark(:,1), Benchmark(:,2),'-*b','LineWidth',0.5,'MarkerSize',1);
% lg=legend('Abaqus','Benchmark','Location','northwest');
% P_hydro(:,1)=P_2ms_plane(:,1);
% P_hydro(:,2)=abs(P_hydro(:,1))*1025*9.81+1.01e5; 
plot(case1_average(:,1), case1_average(:,2),'-r','LineWidth',0.5);
hold on
plot(case1_consistent(:,1), case1_consistent(:,2),'-.b','LineWidth',0.5);
hold on
plot(case3_consistent(:,1), case3_consistent(:,2),'--g','LineWidth',0.5);
lg=legend('Average','Consistent','Benchmark','Location','south');
set(lg,'FontSize',5) 
xlim([0,300]);
%ylim([-5,10]); 
lb1=xlabel('Frequency (Hz)');
lb2=ylabel('Pseudo velocity (m/s)');
% lb1=xlabel('Location (m)');
% lb2=ylabel('Total pressure (Pa)');
set(lb1,'FontSize',20);
set(lb2,'FontSize',20);
axis square
set(gca,'fontsize',10)
